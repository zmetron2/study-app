# Post 1: Phaser this binding issue
$body1 = @{
  title = "Phaser에서 WebSocket 이벤트 안에서 this가 사라지는 이유"
  description = @"
Phaser Scene 내부에서 WebSocket 이벤트 콜백을 등록했는데, 콜백 함수 안에서 this.add나 this.physics 같은 Scene 메서드를 호출하면 undefined 오류가 발생하는 문제를 다룹니다. 비동기 콜백에서 Phaser 캐릭터가 움직이지 않던 근본 원인과 해결책을 공유합니다.
...
create() {
  ...
  fetch('/api/map-data').then((res) => res.json()).then((mapData) => {
...
$response1 = Invoke-RestMethod -Uri "https://zmetron-study.pages.dev/api/resources" -Method POST -ContentType "application/json; charset=utf-8" -Body ([System.Text.Encoding]::UTF8.GetBytes($body1))
Write-Host "Post 1 Response:" ($response1 | ConvertTo-Json)

Start-Sleep -Seconds 2

# Post 2: Next.js build/deploy error collection
$body2 = @{
  title = "Next.js에서 서버 폴더 때문에 빌드가 실패한 이유"
  description = @"
Next.js 프로젝트에서 src/server 폴더를 추가하거나 관련 의존성을 설치했을 때 빌드가 실패하는 원인을 분석하고, tsconfig exclude 설정과 next.config.ts 조정으로 배포 파이프라인을 복구하는 방법을 정리합니다.
...
$response2 = Invoke-RestMethod -Uri "https://zmetron-study.pages.dev/api/resources" -Method POST -ContentType "application/json; charset=utf-8" -Body ([System.Text.Encoding]::UTF8.GetBytes($body2))
Write-Host "Post 2 Response:" ($response2 | ConvertTo-Json)
