...
11. **Resources Content Writing Rules (Agent Standard)**:
   ...
   - **Table Design**: 표준 마크다운 표 문법을 사용하되, 시스템에서 정의한 특수 스타일(상하단 굵은 선, 내부 점선 등)이 적용되도록 정확한 문법을 유지합니다. (표 항목은 '무료여부', '기능', '활용도' 등을 권장하며, 카테고리는 '문서/가이드', '개발도구', '디자인/UIUX', '서버', '도메인', '이슈', 'API', '기타'를 엄격히 준수함)

## 🚀 CI/CD Flow
- **Definition of 'Deploy'**: 이 프로젝트에서 '배포해줘'라는 요청은 로컬 빌드가 아닌 **Git Push를 통한 Cloudflare Pages 자동 배포**를 의미합니다. Windows 환경의 로컬 빌드 제약을 피하기 위해 항상 이 방식을 우선합니다.
